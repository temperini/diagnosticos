...
const RANGE_BY_ANSWER = Object.freeze({
  ...
  orcamento_projeto_credito: "fundingProjectBudget",
});
...
function fundingCapEstimate(stage) {
  const config = PARAMETERS.funding.nonReimbursableByStage[stage];
  if (!config) return null;

  return {
    kind: "financial",
    display: "up_to",
    label: "Potencial de captação MonyU",
    max: config.cap,
    period: "por projeto",
    premise: `Régua MonyU para ${config.label}. Representa potencial de captação em série para o projeto, condicionado a tema, região, aderência, disponibilidade e concorrência das chamadas. Não é valor garantido.`,
  };
}

function percentageEstimate(label, rate, premise) {
  return { kind: "percentage", label, rate, premise };
}

function creditEstimate(range) {
  if (!range) return null;

  const rate = PARAMETERS.funding.creditReferenceRate;
  const min = Math.round(range.min * rate);
  const max = range.max === null ? null : Math.round(range.max * rate);

  return {
    kind: "financial",
    display: range.min === 0 ? "up_to" : "range",
    label: "Parcela financiável de referência",
    min,
    max,
    period: "por projeto",
    premise: `Simulação conservadora de ${(rate * 100).toFixed(0)}% sobre o orçamento informado. A aprovação depende de linha disponível, itens financiáveis, garantias, prazo, taxa e análise de crédito.`,
  };
}

function equityEstimate(stage) {
  const config = PARAMETERS.funding.equityTicketByStage[stage];
  if (!config) return null;

  return {
    kind: "financial",
    display: "reference",
    label: "Ticket de referência para equity",
    max: config.value,
    period: "por rodada",
    premise: `Referência conservadora para ${config.label}. Capital de risco é captado pela empresa, não vinculado automaticamente a um projeto, e envolve diluição, valuation, tese e negociação com investidores.`,
  };
}
...
function fitLevelFromStage(profile, answers) {
  ...
  if (["validacao", "operacao", "tracao"].includes(answers.estagio_projeto)) {
    return profile.regularityPositive ? "alto" : "medio";
  }
  if (["ideacao", "escala"].includes(answers.estagio_projeto)) return "medio";
  return "baixo";
}
...
export function isQuestionVisible(question, answers, profile = deriveProfile(answers)) {
  ...
  switch (question.condition) {
    ...
    case "credit_budget_track":
      return (
        profile.innovationPositive &&
        FUNDING_OBJECTIVES.has(answers.objetivo_principal) &&
        ["sim", "talvez"].includes(answers.apetite_divida) &&
        ["sim", "talvez"].includes(answers.capacidade_credito)
      );
...
function evaluateFomento(answers, profile) {
  const fit =
    profile.innovationPositive &&
    Boolean(PARAMETERS.funding.nonReimbursableByStage[answers.estagio_projeto]);
  ...
  return createMechanism("FOMENTO_NAOREEMB", {
    ...
    estimate:
      status === STATUS.NOT_RELEVANT
        ? null
        : fundingCapEstimate(answers.estagio_projeto) ??
...
function evaluateCredit(answers, profile) {
  ...
  return createMechanism("CREDITO_SUBSIDIADO", {
    ...
    estimate:
      status === STATUS.NOT_RELEVANT
        ? null
        : creditEstimate(getAnswerRange("orcamento_projeto_credito", answers.orcamento_projeto_credito)) ??
          qualitativeEstimate("Linha a dimensionar", "Informe o orçamento do projeto para aproximarmos a parcela financiável. A aprovação depende de garantias, prazo, taxa e linha disponível."),
    positives: unique([
...
function evaluateEmbrapii(answers, profile) {
  const fit =
    ...
    ["validacao", "operacao", "tracao", "escala"].includes(answers.estagio_projeto);
  ...
  return createMechanism("EMBRAPII", {
    ...
    estimate:
      fit
        ? percentageEstimate(
            "Apoio EMBRAPII de referência",
            PARAMETERS.funding.embrapiiReferenceRate,
            "Referência de 33% do valor do projeto. A composição final é negociada com a Unidade EMBRAPII; para MPEs e startups, parcerias específicas podem reduzir a contrapartida da empresa conforme regras vigentes.",
          )
        : null,
    positives: unique([
...
function evaluateStartup(answers, profile) {
  ...
  return createMechanism("STARTUPLC182", {
    ...
    estimate:
      status === STATUS.NOT_RELEVANT
        ? null
        : equityEstimate(answers.estagio_projeto) ??
          qualitativeEstimate("Fit de estruturação", "Capital de risco depende de tese, diluição, valuation, equipe e evidências de mercado. O Marco Legal não é um benefício financeiro direto."),
    positives: unique([
...
function valueScore(result) {
  ...
  if (result.estimate?.kind === "percentage") {
    return result.estimate.rate >= 0.5 ? 28 : 22;
  }
