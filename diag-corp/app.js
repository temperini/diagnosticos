...
const elements = {
  ...
  contactReference: document.getElementById("contactReference"),
...
function formatFinancialEstimate(estimate) {
  ...
  const period = estimate.period ?? "por ano";
  if (estimate.display === "up_to") return `Até ${formatMoney(estimate.max)} ${period}`;
  if (estimate.display === "reference") return `Referência de ${formatMoney(estimate.max)} ${period}`;
  if (estimate.max === null) return `A partir de ${formatMoney(estimate.min)} ${period}`;
  return `Entre ${formatMoney(estimate.min)} e ${formatMoney(estimate.max)} ${period}`;
}

function formatPercentageEstimate(estimate) {
  if (!estimate || estimate.kind !== "percentage") return "";
  return `Cerca de ${(estimate.rate * 100).toFixed(0)}% do valor do projeto`;
}
...
function renderEstimate(estimate) {
  ...
  if (estimate.kind === "percentage") {
    return `
      <div class="estimate-block">
        <span class="estimate-label">${escapeHtml(estimate.label)}</span>
        <strong>${escapeHtml(formatPercentageEstimate(estimate))}</strong>
      </div>
    `;
  }
...
function renderAssumptions(diagnostic) {
  const financialMechanisms = diagnostic.mechanisms.filter(
    (mechanism) => ["financial", "percentage"].includes(mechanism.estimate?.kind),
  );
...
function renderResult() {
  ...
  elements.resultIntro.textContent = diagnostic.noInnovation
    ...
    : "Organizamos as alavancas financeiras por relevância, potencial, esforço e confiança das informações.";
...
function createDiagnosticId() {
  ...
  return `monyu-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}

function mechanismSummary(mechanism) {
  const status = STATUS_COPY[mechanism.status]?.label ?? mechanism.status;
  const estimate = mechanism.estimate?.kind === "financial"
    ? formatFinancialEstimate(mechanism.estimate)
    : mechanism.estimate?.kind === "percentage"
      ? formatPercentageEstimate(mechanism.estimate)
      : mechanism.estimate?.label ?? "";

  return {
    name: mechanism.name,
    status,
    estimate,
    nextStep: mechanism.nextStep ?? "",
  };
}
...
function getContactDetails() {
  return {
    ...
    role: elements.contactRole.value.trim(),
    company: elements.contactCompany.value.trim(),
    email: elements.contactEmail.value.trim(),
    website: normalizeOptionalUrl(elements.contactWebsite.value),
    reference: normalizeOptionalUrl(elements.contactReference.value),
    notes: elements.contactNotes.value.trim(),
  };
}

function buildWhatsAppMessage(contact, diagnostic, profileSummary) {
  const prioritized = diagnostic.mainOpportunities.length
    ? diagnostic.mainOpportunities.map((mechanism) => mechanism.name)
    : diagnostic.preparations.map((mechanism) => mechanism.name);
  const profile = profileSummary.signals.map((signal) => signal.value).join(" | ");
  const lines = [
    "Olá, MonyU. Concluí o diagnóstico e gostaria de validar o potencial de captação dos projetos da minha empresa.",
    "",
    `Respondente: ${contact.name}`,
    `Cargo: ${contact.role}`,
    `Empresa: ${contact.company}`,
    `E-mail: ${contact.email}`,
    "",
    `Perfil identificado: ${profile}`,
    `Resultado principal: ${diagnostic.recommendation.heading}`,
    `Oportunidades priorizadas: ${prioritized.join("; ") || "Mapeamento inicial"}`,
  ];

  if (contact.website) lines.push(`Site: ${contact.website}`);
  if (contact.reference) lines.push(`Link complementar: ${contact.reference}`);
  if (contact.notes) lines.push(`Observações: ${contact.notes}`);

  lines.push(
    "",
    "Quero entender os próximos passos para transformar essas alavancas em resultado financeiro para a inovação.",
  );

  return lines.join("\n");
}

function buildDiagnosticPayload(contact, diagnostic, profileSummary) {
  const mainOpportunities = diagnostic.mainOpportunities.map(mechanismSummary);
  const preparations = diagnostic.preparations.map(mechanismSummary);

  return {
    event: "diagnostic_completed",
    diagnosticId: createDiagnosticId(),
    submittedAt: new Date().toISOString(),
    rulesVersion: APP_META.rulesVersion,
    source: {
    ...
    result: {
      profile: profileSummary.signals.map((signal) => ({
        label: signal.label,
        value: signal.value,
        detail: signal.detail,
      })),
      innovation: profileSummary.innovation,
...
function sendDiagnosticToSheet(payload) {
  const endpoint = window.MONYU_CAPTURE_CONFIG?.endpoint;
...
function submitContactForm(event) {
  ...
  if (window.MONYU_CAPTURE_CONFIG?.turnstileSiteKey && !turnstileToken) {
    showToast("Confirme a verificação de segurança para continuar.");
    return;
  }
  ...
  const diagnostic = evaluateDiagnostic(state.answers);
  const profileSummary = buildProfileSummary(state.answers, diagnostic.profile);
  const message = buildWhatsAppMessage(contact, diagnostic, profileSummary);
  const whatsappUrl = `https://wa.me/5585991577137?text=${encodeURIComponent(message)}`;
  const popup = window.open(whatsappUrl, "_blank", "noopener,noreferrer");
  if (!popup) window.location.assign(whatsappUrl);

  void sendDiagnosticToSheet(buildDiagnosticPayload(contact, diagnostic, profileSummary));
  if (turnstileWidgetId !== null && window.turnstile) {
    window.turnstile.reset(turnstileWidgetId);
    turnstileToken = "";
  }
  showToast("Resumo preparado. Continue a conversa no WhatsApp.");
}

function initializeTurnstile() {
  const siteKey = window.MONYU_CAPTURE_CONFIG?.turnstileSiteKey;
  if (!siteKey) return;
  ...
  script.onload = () => {
    turnstileWidgetId = window.turnstile.render(elements.turnstileContainer, {
      ...
      "error-callback": () => {
        turnstileToken = "";
        showToast("A verificação de segurança não foi carregada. Tente novamente.");
      },
    });
  };
...
function buildExportText() {
  ...
  if (diagnostic.noInnovation) {
    ...
    diagnostic.mainOpportunities.forEach((mechanism) => {
      ...
      if (mechanism.estimate?.kind === "percentage") lines.push(`  ${formatPercentageEstimate(mechanism.estimate)}`);
...
elements.privacyButton.addEventListener("click", () => {
  if (elements.resultView.hidden) {
    showToast("O detalhamento sobre dados aparece no encerramento do diagnóstico.");
    return;
  }
  elements.privacyDetails.open = true;
  elements.privacyDetails.scrollIntoView({ behavior: "smooth", block: "center" });
  window.setTimeout(() => elements.privacyDetails.querySelector("summary").focus(), 250);
});
